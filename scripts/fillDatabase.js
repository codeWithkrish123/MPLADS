...
async function fillDatabase() {
  console.log("=================================================");
  console.log("🚀 Starting Full Database Population for MPLADS");
  console.log("=================================================");
  ...
  try {
    ...
    // 2. Populate 543 MPs
    console.log(`📥 Inserting ${SEEDED_MPS.length} Lok Sabha MP Allocations...`);
    await client.query("DELETE FROM mp_allocations");
    for (const mp of SEEDED_MPS) {
      await client.query(
        "INSERT INTO mp_allocations (id, sr_no, state, mp_name, constituency, allocated_amount) VALUES ($1, $2, $3, $4, $5, $6)",
        [mp.id, mp.sr_no, mp.state, mp.mp_name, mp.constituency, mp.allocated_amount]
      );
    }
    console.log(`✅ 543 Lok Sabha MPs successfully populated into PostgreSQL!`);

    // 3. Populate 12 Calamity Consents
    console.log(`📥 Inserting ${SEEDED_CALAMITIES.length} Disaster Relief Calamity Consents...`);
    await client.query("DELETE FROM calamity_consents");
    for (const cal of SEEDED_CALAMITIES) {
      await client.query(
        "INSERT INTO calamity_consents (id, sr_no, calamity_type, calamity_name, mp_name, consent_date, consent_amount) VALUES ($1, $2, $3, $4, $5, $6)",
        [cal.id, cal.sr_no, cal.calamity_type, cal.calamity_name, cal.mp_name, cal.consent_date, cal.consent_amount]
      );
    }
    console.log(`✅ 12 Disaster Relief Calamity Consents successfully populated into PostgreSQL!`);

    // 4. Populate Users
    console.log("📥 Inserting Demo Government Users...");
    await client.query("DELETE FROM users");
    const demoUsers = [
      { id: "usr-ministry-01", email: "ministry@mplads.gov.in", password_hash: "$2a$10$w8.dPn939YVbM7h3vFz2wOaE4e8xkJ4fM2m0O1aG8Z6wD4qE6v8x.", full_name: "National Administrator (MoSPI)", role: "MINISTRY" },
      { id: "usr-state-mh-01", email: "state.maharashtra@mplads.gov.in", password_hash: "$2a$10$w8.dPn939YVbM7h3vFz2wOaE4e8xkJ4fM2m0O1aG8Z6wD4qE6v8x.", full_name: "State Nodal Officer Maharashtra", role: "STATE_NODAL", state: "Maharashtra" },
      { id: "usr-state-up-01", email: "state.up@mplads.gov.in", password_hash: "$2a$10$w8.dPn939YVbM7h3vFz2wOaE4e8xkJ4fM2m0O1aG8Z6wD4qE6v8x.", full_name: "State Nodal Officer Uttar Pradesh", role: "STATE_NODAL", state: "Uttar Pradesh" },
      { id: "usr-dist-amr-01", email: "district.amravati@mplads.gov.in", password_hash: "$2a$10$w8.dPn939YVbM7h3vFz2wOaE4e8xkJ4fM2m0O1aG8Z6wD4qE6v8x.", full_name: "District Collector Amravati", role: "DISTRICT_AUTHORITY", state: "Maharashtra", district: "Amravati" },
      { id: "usr-mp-var-01", email: "mp.varanasi@mplads.gov.in", password_hash: "$2a$10$w8.dPn939YVbM7h3vFz2wOaE4e8xkJ4fM2m0O1aG8Z6wD4qE6v8x.", full_name: "Shri Narendra Modi (Varanasi MP)", role: "MP", state: "Uttar Pradesh", constituency: "VARANASI" },
      { id: "usr-citizen-01", email: "citizen@mplads.gov.in", password_hash: "$2a$10$w8.dPn939YVbM7h3vFz2wOaE4e8xkJ4fM2m0O1aG8Z6wD4qE6v8x.", full_name: "Public Citizen / Social Auditor", role: "CITIZEN" },
    ];
    for (const u of demoUsers) {
      await client.query(
        "INSERT INTO users (id, email, password_hash, full_name, role, state, district, constituency) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)",
        [u.id, u.email, u.password_hash, u.full_name, u.role, u.state || null, u.district || null, u.constituency || null]
      );
    }
    console.log("✅ Demo Users populated into PostgreSQL!");

    // 5. Populate Compliance Rules
    console.log("📥 Inserting Statutory Compliance Policies...");
    ...
    console.log("✅ Compliance Rules populated into PostgreSQL!");

    // 6. Populate Implementing Agencies
    console.log("📥 Inserting Implementing Agencies...");
    ...
    console.log("✅ Implementing Agencies populated into PostgreSQL!");

    // 7. Populate Alerts
    console.log("📥 Inserting Baseline Risk Alerts...");
    ...
    console.log("✅ Risk Alerts populated into PostgreSQL!");

    // 8. Populate Duplicate Reviews
    console.log("📥 Inserting Near Duplicate Reviews...");
    ...
    console.log("✅ Near Duplicate Reviews populated into PostgreSQL!");
    ...
    console.log("✅ Audit Ledger initialized with Genesis Hash!");
    ...
    console.log(`📊 Total Lok Sabha MPs: ${mpCount.rows[0].count}`);
    console.log(`📊 Total Calamity Consents: ${calCount.rows[0].count}`);
    console.log(`📊 Total Compliance Rules: ${ruleCount.rows[0].count}`);
    console.log(`📊 Total Agencies: ${agyCount.rows[0].count}`);
    console.log(`📊 Total Risk Alerts: ${altCount.rows[0].count}`);
    console.log(`📊 Total Users: ${usrCount.rows[0].count}`);
    console.log("=================================================");
  } finally {
    client.release();
    await pool.end();
